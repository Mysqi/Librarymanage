...
import static com.mysql.qi_fu.librarymanage.calendar.CompactCalendarView.FILL_LARGE_INDICATOR;
import static com.mysql.qi_fu.librarymanage.calendar.CompactCalendarView.NO_FILL_LARGE_INDICATOR;
import static com.mysql.qi_fu.librarymanage.calendar.CompactCalendarView.SMALL_INDICATOR;
...
class CompactCalendarController {
	...
	private boolean useTwoLetterAbbreviation = false;
	...
	private String[] dayColumnNames = new String[]{"一", "二", "三", "四", "五", "六", "日"};
	...
	private int eventTextSize = 24;
	private int weekDayTextSize = 26;
	...
	CompactCalendarController(Paint dayPaint, OverScroller scroller, Rect textSizeRect, AttributeSet attrs,
	                          ...
	                          Locale locale, TimeZone timeZone, CompactCalendarView compactCalendarView) {
		this.dayPaint = dayPaint;
		this.scroller = scroller;
		this.textSizeRect = textSizeRect;
		this.currentDayBackgroundColor = currentDayBackgroundColor;
		this.currentDayTextColor = currentDayBackgroundColor;
	...
	private void loadAttributes(AttributeSet attrs, Context context) {
		if (attrs != null && context != null) {
			...
			try {
				...
				eventTextColor = typedArray.getColor(R.styleable.CompactCalendarView_compactCalendarEventTextColor, calenderTextColor);
				...
				weekBackgroundColor = typedArray.getColor(R.styleable.CompactCalendarView_compactCalendarWeekBackgroundColor, calenderBackgroundColor);
				weekDayTextColor = typedArray.getColor(R.styleable.CompactCalendarView_compactCalendarWeekTextColor, calenderTextColor);
				weekendDayTextColor = typedArray.getColor(R.styleable.CompactCalendarView_compactCalendarWeekendTextColor, calenderTextColor);
				...
				eventTextSize = typedArray.getDimensionPixelSize(R.styleable.CompactCalendarView_compactCalendarEventTextSize,
				...
				heightPerDay = typedArray.getDimensionPixelSize(R.styleable.CompactCalendarView_compactCalendarDayHeight,
						(int) TypedValue.applyDimension(TypedValue.COMPLEX_UNIT_DIP, heightPerDay, context.getResources().getDisplayMetrics()));
				...
				isShown = typedArray.getBoolean(R.styleable.CompactCalendarView_compactCalendarIsShown, true);
			} finally {
				typedArray.recycle();
			}
		}
	...
	private void init(Context context) {
		...
		compactCalendarView.adjustHeight();
	}
	...
	void setCurrentSelectedDayTextColor(int currentSelectedDayBackgroundColor) {
		this.currentSelectedDayIndicatorStyle = currentSelectedDayBackgroundColor;
	}
	...
	void setUseWeekDayAbbreviation(boolean useThreeLetterAbbreviation) {
		...
		if (useThreeLetterAbbreviation) {
			this.dayColumnNames = new String[]{"星期一", "星期二", "星期三", "星期四", "星期五", "星期六", "星期日"};
		}
...
//		heightPerDay = targetHeight > 0 ? targetHeight / 7 : height / 7;
	...
	public boolean shouldShowNextMonth() {
		if ((currentCalender.get(Calendar.YEAR) == todayCalender.get(Calendar.YEAR)) && (currentCalender.get(Calendar.MONTH) >= todayCalender.get(Calendar.MONTH))) {
			return false;
		}
	...
	int getActualHeight() {
		int weekCount = countWeekOfMonth();
		int actualHeight = getHeightPerDay()*(weekCount+1);
		return actualHeight;
	}

	public int countWeekOfMonth() {
		return currentCalender.getActualMaximum(Calendar.WEEK_OF_MONTH);
	}
			...
			for (int i = 0; i < uniqEvents.size(); i++) {
				...
				if (shouldDrawIndicatorsBelowSelectedDays || (!shouldDrawIndicatorsBelowSelectedDays && !isSameDayAsCurrentDay && !isCurrentSelectedDay) || animationStatus == EXPOSE_CALENDAR_ANIMATION) {
					if (eventIndicatorStyle == FILL_LARGE_INDICATOR || eventIndicatorStyle == NO_FILL_LARGE_INDICATOR) {
						...
						drawEventIndicatorCircle(canvas, xPosition, yPosition, event.getColor(), (String)event.getData());
					} else {
	...
	private void drawTwoEvents(Canvas canvas, float xPosition, float yPosition, List<Event> eventsList) {
		...
		drawEventIndicatorCircle(canvas, xPosition + (xIndicatorOffset * -1), yPosition, eventsList.get(0).getColor(), (String)eventsList.get(0).getData());
		//draw second event just right of center
		drawEventIndicatorCircle(canvas, xPosition + (xIndicatorOffset * 1), yPosition, eventsList.get(1).getColor(), (String)eventsList.get(1).getData());
	}
	...
	private void drawEventsWithPlus(Canvas canvas, float xPosition, float yPosition, List<Event> eventsList) {
		...
		for (int j = 0, k = -2; j < 3; j++, k += 2) {
			...
			if (j == 2) {
				...
				drawEventIndicatorCircle(canvas, xStartPosition, yPosition, event.getColor(), (String)event.getData());
			}
		}
	}
	...
	void drawMonth(Canvas canvas, Calendar monthToDrawCalender, int offset) {
		...
		for (int dayColumn = 0, dayRow = 0; dayColumn <= 6; dayRow++) {
			...
			boolean isWeekend = dayColumn==5||dayColumn==6 ? true : false;
			if (dayRow == 0) {
				// first row, so draw the first letter of the day
				if (shouldDrawDaysHeader) {
					dayPaint.setTypeface(Typeface.DEFAULT_BOLD);
					dayPaint.setStyle(Paint.Style.FILL);
					dayPaint.setColor(isWeekend ? weekendDayTextColor : weekDayTextColor);
					dayPaint.setTextSize(weekDayTextSize);
				...
				if (day <= 0) {
					...
					if (isCurrentDay) {
						...
						dayPaint.setColor(weekendDayTextColor);
					} else if (isSameMonthAsToday && day>todayDayOfMonth) {
						dayPaint.setColor(weekendDayTextColor);
					} else {
						dayPaint.setColor(calenderTextColor);
					}
	...
	private void drawEventIndicatorCircle(Canvas canvas, float x, float y, int color, String text) {
		dayPaint.setColor(color);
		drawEventText(canvas, text, x, y);
...
//		}
	...
	private void drawEventText(Canvas canvas, String text, float x, float y) {
		dayPaint.setTextSize(eventTextSize);
		dayPaint.setColor(eventTextColor);
		canvas.drawText(text, x-2, y+8, dayPaint);
		dayPaint.setTextSize(textSize);
	}
}